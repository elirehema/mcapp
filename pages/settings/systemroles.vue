<template>
  <div v-if="$rules.hasPermission('role.read')">
    <v-data-table
      v-if="roles "
      :headers="headers"
      :items="roles"
        ...
        <v-toolbar color="primary" flat>
          ...
          <v-dialog v-if="$rules.hasPermission('role.add')" v-model="dialog" max-width="650px">
            ...
            <v-card>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="warning darken-1" small  @click="close"> Cancel </v-btn>
                <v-btn color="button darken-1" small @click="save"> Save </v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
